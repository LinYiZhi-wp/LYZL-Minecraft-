            ...
            if (e.Uri.StartsWith(_redirectUri, StringComparison.OrdinalIgnoreCase))
            {
                ...
                try 
                {
                    ...
                    else if (!string.IsNullOrEmpty(query["error"])) 
                    {
                        ...
                        MessageBox.Show($"Login error: {query["error_description"] ?? query["error"]}", "Login Failed");
                ...
                {
                    MessageBox.Show($"Error parsing redirect: {ex.Message}", "Internal Error");
                    this.DialogResult = false;
                    this.Close();
                }
            }
...
ê*cascade082[file:///C:/Users/Linyizhi/.gemini/GeminiLauncher/Views/Dialogs/MicrosoftLoginDialog.xaml.cs